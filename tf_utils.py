...
def h_distance(p, q):
    ...
    return tf.multiply(tf.sqrt(tf.reduce_sum(tf.square(tf.sqrt(p)-tf.sqrt(q)), axis=1)), 1/np.sqrt(2))
...
def get_distance_fn(dis_type:str):
    if dis_type.lower() == 'kl':
        ...
        return sl_distance
    elif dis_type.lower() == 'sig':
        return tf.losses.sigmoid_cross_entropy
    else:
        raise ValueError("Unrecognized distance type")


def compute_e_and_belief_distance(e, b, dis_type):
    distance_fn = get_distance_fn(dis_type=dis_type)
    if dis_type.lower() == 'sl' or dis_type.lower() == 'sig':
...
def softmax_with_tau(logits, tau):
    tau = tf.constant(tau)
    p = tf.exp(logits/tau)/tf.reduce_sum(tf.exp(logits/tau), axis=None)
    return p
